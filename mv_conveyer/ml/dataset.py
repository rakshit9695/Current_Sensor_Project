"""
mv_conveyer/ml/dataset.py
=========================
PyTorch Dataset for the MV Conveyer iTransformer.

Each sample returns a look-back window of normalised current readings and
the corresponding multi-task labels (same structure as wave_soldering).

Input tensor  : (look_back, 4)   – I1, I2, I3, I_avg (z-score normalised)
State label   : int  0=HALT / 1=IDLE / 2=RUNNING
...
class ConveyorDataset(Dataset):
    """
    Sliding look-back window dataset for MV Conveyer.
...
def build_datasets(
    ...
    train_frac:  float = 0.70,
    val_frac:    float = 0.15,
) -> tuple[ConveyorDataset, ConveyorDataset, ConveyorDataset, NormStats]:
    train_df, val_df, test_df = temporal_split(df_labeled, train_frac, val_frac)
    norm_stats = NormStats.from_dataframe(train_df)
    train_ds = ConveyorDataset(train_df, look_back=look_back, norm_stats=norm_stats)
    val_ds   = ConveyorDataset(val_df,   look_back=look_back, norm_stats=norm_stats)
    test_ds  = ConveyorDataset(test_df,  look_back=look_back, norm_stats=norm_stats)
