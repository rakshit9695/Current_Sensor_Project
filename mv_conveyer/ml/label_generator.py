"""
mv_conveyer/ml/label_generator.py
==================================
Auto-labeling pipeline for MV Conveyer data. No manual annotation required.

Three states
------------
  0 = HALT    (RED)   : machine completely stopped, I_Avg < halt_max
  1 = IDLE    (AMBER) : other equipment running, conveyor motor off
                        I_Avg in [halt_max, run_min)
  2 = RUNNING (GREEN) : conveyor motor active, all 3 phases loaded
                        I_Avg >= run_min

Cycle definition
----------------
A "cycle" is one contiguous RUNNING segment (minimum min_run samples).
  - cycle_id  : integer index of the running segment (0-based)
  - cycle_pos : 0.0 → 1.0 fractional position within the RUNNING segment

The IDLE/HALT gaps between running segments do NOT receive a cycle_id
(they are set to -1 / NaN), which matches the wave_soldering convention.

Job clustering
--------------
K-means on per-cycle features separates distinct operational modes:
  - Short cyclic runs (~11 s): conveyor runs periodically (Cyclic_Run data)
  - Long continuous runs (60+ s): belt runs non-stop (Stop_and_Run data)

With n_job_types=2 these two modes separate cleanly. If n_job_types=3
a third cluster may capture edge cases (startup transients, etc.).
"""
...
STATE_NAMES = {HALT: "HALT", IDLE: "IDLE", RUNNING: "RUNNING"}

# ── Default thresholds (calibrated from actual data) ──────────────────────────
DEFAULT_HALT_MAX  = 0.10   # A  I_Avg < 0.10 → HALT
DEFAULT_RUN_MIN   = 0.35   # A  I_Avg >= 0.35 → RUNNING
DEFAULT_MIN_RUN   = 3      # samples  minimum RUNNING segment length
