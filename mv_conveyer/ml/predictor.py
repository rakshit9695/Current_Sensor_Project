...
Columns added to a copy of the input DataFrame:
  ml_state        : int   0/1/2   (HALT / IDLE / RUNNING)
  ml_state_name   : str   "HALT" / "IDLE" / "RUNNING"
  ml_state_conf   : float 0-1     (softmax probability of predicted class)
  ml_cycle_pos    : float 0-1     (predicted position in current RUNNING segment)
  ml_job_type     : int   0..K-1  (predicted operational mode cluster)
  ml_job_name     : str   "Cyclic" / "Continuous" / ...
"""
...
class ConveyorPredictor:
    ...
    Usage
    -----
        predictor = ConveyorPredictor.from_checkpoint()
    ...
    @classmethod
    def from_checkpoint(cls, path: Path = CHECKPOINT_PATH) -> "ConveyorPredictor":
    ...
    def evaluate_on_labeled(self, df_labeled: pd.DataFrame, batch_size: int = 256) -> dict:
        ...
        state_report = classification_report(
            ...
            labels=[0, 1, 2],
            target_names=["HALT", "IDLE", "RUNNING"],
            output_dict=True,
            zero_division=0,
        )
