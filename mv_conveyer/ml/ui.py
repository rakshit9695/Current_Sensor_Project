"""
mv_conveyer/ml/ui.py
====================
Streamlit ML page for MV Conveyer.
...
from .label_generator import generate_labels, DEFAULT_HALT_MAX, DEFAULT_RUN_MIN, DEFAULT_MIN_RUN, DEFAULT_N_JOBS
...
from .predictor import ConveyorPredictor

_STATE_COLORS = {0: "#e74c3c", 1: "#f39c12", 2: "#27ae60"}
_STATE_NAMES  = {0: "HALT", 1: "IDLE", 2: "RUNNING"}
...
def _ss_set(key, val):
    st.session_state[f"mvc_ml_{key}"] = val


# ── Main entry point ──────────────────────────────────────────────────────────

def render_ml_page(df: pd.DataFrame, cyclic_df: pd.DataFrame | None = None):
    """
    Render the full ML diagnostics page for MV Conveyer.

    Parameters
    ----------
    df        : combined DataFrame (Stop_and_Run + Cyclic_Run) —
                provides lookback context at the boundary between the two files.
    cyclic_df : Cyclic_Run only — used for label generation and state targets.
                Labels are generated from cyclic_df; the combined df is used for
                dataset construction so lookback windows can reach into Stop_and_Run.
    """
    st.title("🤖 ML Diagnostics — MV Conveyer")
    st.caption(
        "Labels generated from **Cyclic_Run data** only. "
        "Combined dataset (Stop_and_Run + Cyclic_Run) used for training so the "
        "look-back window has full context at the session boundary."
    )
    ...
    with tab1:
        _tab_data_labels(cyclic_df)
    with tab2:
        _tab_train(df, cyclic_df)
    with tab3:
        _tab_evaluate(cyclic_df)
    with tab4:
        _tab_inference(cyclic_df)
...
def _no_cyclic_warning():
    st.warning(
        "Cyclic_Run data not found in session. "
        "Go to **Data Validation**, upload **both** CSV files "
        "(make sure one filename contains 'cyclic'), then return here."
    )
...
def _tab_data_labels(df: pd.DataFrame | None):
    ...
    with st.expander("⚙️ Threshold parameters", expanded=False):
        ...
        halt_max = c1.number_input("HALT max I_Avg (A)", min_value=0.01, max_value=0.20,
                                   value=DEFAULT_HALT_MAX, step=0.01, key="mvc_halt_max")
        run_min  = c2.number_input("RUN min I_Avg (A)",  min_value=0.10, max_value=0.80,
                                   value=DEFAULT_RUN_MIN,  step=0.01, key="mvc_run_min")
        min_run  = c3.number_input("Min run samples",    min_value=1,    max_value=30,
                                   value=DEFAULT_MIN_RUN,  step=1,    key="mvc_min_run")
        n_jobs   = c4.number_input("Job clusters (K)",   min_value=2,    max_value=5,
                                   value=DEFAULT_N_JOBS,   step=1,    key="mvc_n_jobs")

    regen = st.button("🔄 (Re)generate Labels", key="mvc_regen_labels")
    ...
    st.markdown("---")
    n_total  = len(df_lab)
    n_halt   = (df_lab["state_label"] == 0).sum()
    n_idle   = (df_lab["state_label"] == 1).sum()
    n_run    = (df_lab["state_label"] == 2).sum()
    n_cycles = meta["n_cycles"]

    c1, c2, c3, c4, c5 = st.columns(5)
    c1.metric("Total Samples",  f"{n_total:,}")
    c2.metric("HALT (RED)",     f"{n_halt:,}",  f"{100*n_halt/n_total:.1f}%")
    c3.metric("IDLE (AMBER)",   f"{n_idle:,}",  f"{100*n_idle/n_total:.1f}%")
    c4.metric("RUNNING (GREEN)",f"{n_run:,}",   f"{100*n_run/n_total:.1f}%")
    ...
    with col_pie:
        fig_pie = go.Figure(go.Pie(
            labels=["HALT", "IDLE", "RUNNING"],
            values=[n_halt, n_idle, n_run],
    ...
    with col_hist:
        ...
        if durs:
            ...
            fig_hist.update_layout(
                title="RUNNING Segment Durations (s)",
    ...
    if cf_dict and "cycle_id" in cf_dict:
        ...
        if "job_type" in cf.columns and len(cf) > 0:
            st.subheader("Operational Mode Clusters (per RUNNING segment)")
            fig_sc = go.Figure()
            for jt in sorted(cf["job_type"].unique()):
                ...
                fig_sc.add_trace(go.Scatter(
                    ...
                    text=[f"Duration: {d:.0f}s<br>I_avg: {i:.3f}A" for d, i in zip(sub["duration_s"], sub["mean_iavg"])],
                    hoverinfo="text+name",
                ))
            fig_sc.update_layout(
                title="Cluster: Duration vs Mean Current per RUNNING Segment",
            ...
            for jt in sorted(cf["job_type"].unique()):
                sub = cf[cf["job_type"] == jt]
                label = {0: "Light Load (lower-current bursts)", 1: "Heavy Load (sustained/high-current runs)"}.get(jt, f"Mode {jt}")
                st.markdown(
...
def _tab_train(df_combined: pd.DataFrame | None, cyclic_df: pd.DataFrame | None = None):
    """
    df_combined : Stop_and_Run + Cyclic_Run — provides full lookback context.
    cyclic_df   : Cyclic_Run only — source of state/cycle/job labels.
    The dataset is built from df_combined but labels come from cyclic_df rows only
    (rows not in cyclic_df receive state=IDLE, cycle_id=-1, job_type=-1).
    """
    if cyclic_df is None:
        _no_cyclic_warning()
        return
    ...
    c1, c2, c3, c4 = st.columns(4)
    look_back  = c1.slider("Look-back window (s)", 10, 120, 30, 5,  key="mvc_look_back")
    epochs     = c2.slider("Epochs",               5,  50,  20, 5,  key="mvc_epochs")
    batch_size = c3.selectbox("Batch size", [32, 64, 128, 256], index=1, key="mvc_batch")
    lr         = c4.select_slider("Learning rate", [1e-4, 3e-4, 1e-3, 3e-3], value=1e-3, key="mvc_lr")

    c5, c6, c7 = st.columns(3)
    d_model    = c5.selectbox("d_model",  [32, 64, 128], index=1, key="mvc_dmodel")
    n_layers   = c6.selectbox("n_layers", [2, 3, 4],     index=1, key="mvc_nlayers")
    n_heads    = c7.selectbox("n_heads",  [2, 4, 8],     index=1, key="mvc_nheads")

    c8, c9, c10 = st.columns(3)
    alpha = c8.number_input("α (state loss weight)",    min_value=0.1, max_value=5.0, value=1.0, step=0.1, key="mvc_alpha")
    beta  = c9.number_input("β (cycle pos weight)",     min_value=0.0, max_value=2.0, value=0.5, step=0.1, key="mvc_beta")
    gamma = c10.number_input("γ (job type weight)",     min_value=0.0, max_value=2.0, value=0.5, step=0.1, key="mvc_gamma")
    ...
    if not st.button("🚀 Start Training", type="primary", key="mvc_train_btn"):
        return

    # ── Build training dataset ────────────────────────────────────────
    # Merge labels (from cyclic df) onto the combined df so that lookback
    # windows at the Start_and_Run → Cyclic_Run boundary are fully populated.
    from mv_conveyer.ml.label_generator import generate_labels as _gen
    import numpy as np

    # Re-label the full combined df: Stop_and_Run rows get IDLE/no-cycle labels
    df_full_lab, _ = _gen(
        df_combined if df_combined is not None else cyclic_df,
        halt_max    = meta.get("halt_max", 0.10),
        run_min     = meta.get("run_min",  0.35),
        min_run     = meta.get("min_run",  3),
        n_job_types = meta.get("n_job_types", 2),
    )
...
def _tab_evaluate(df: pd.DataFrame | None):
    if df is None:
        _no_cyclic_warning()
        return
    st.subheader("Model Evaluation on Cyclic_Run Dataset")
    ...
    if not st.button("▶️ Run Evaluation", type="primary", key="mvc_eval_btn"):
        return

    with st.spinner("Loading model and running inference…"):
        try:
            predictor = ConveyorPredictor.from_checkpoint(CHECKPOINT_PATH)
...
def _tab_inference(df: pd.DataFrame | None):
    ...
    if st.button("🔮 Run Inference on Selection", key="mvc_infer_btn"):
    ...
    for col, color in [("i1","#4e9af1"),("i2","#f1914e"),("i3","#4ef19a"),("i_avg","#f1f14e")]:
        fig.add_trace(go.Scatter(x=t, y=result[col], name=col.upper(),
                                  line=dict(color=color, width=1)), row=1, col=1)

    # Row 2: state comparison
    state_num = {"HALT": 0, "IDLE": 1, "RUNNING": 2}
    ...
    fig.update_yaxes(title_text="State (0-2)",  row=2, col=1,
                     tickvals=[0,1,2], ticktext=["HALT","IDLE","RUN"])
