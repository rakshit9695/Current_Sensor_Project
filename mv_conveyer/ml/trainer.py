"""
mv_conveyer/ml/trainer.py
=========================
Training loop, multi-task loss, and checkpoint management for MV Conveyer.

Multi-task loss
---------------
  L = α·CE(state) + β·MSE(cycle_pos, masked) + γ·CE(job_type, masked)

Samples with cycle_id = -1 (HALT/IDLE gaps between running segments) have
cycle_pos=-1 and job_type=-1. These are masked out so the model is not
penalised for those outputs during non-running periods.

Checkpoint saved to:  mv_conveyer/ml/checkpoints/model.pt
"""
...
from .dataset import ConveyorDataset, NormStats
...
def multi_task_loss(
    ...
    loss_cyc = (
        F.mse_loss(cycle_pos_pred[cyc_mask], cycle_pos_true[cyc_mask])
        if cyc_mask.any() else torch.tensor(0.0, device=state_logits.device)
    )

    job_mask = job_true >= 0
    loss_job = (
        F.cross_entropy(job_logits[job_mask], job_true[job_mask])
        if job_mask.any() else torch.tensor(0.0, device=state_logits.device)
    )
...
def _accuracy(logits, targets, mask=None) -> float:
    preds = logits.argmax(dim=-1)
    if mask is not None:
        return (preds[mask] == targets[mask]).float().mean().item() if mask.any() else float("nan")
...
def train(
    train_ds:   ConveyorDataset,
    val_ds:     ConveyorDataset,
    ...
    *,
    ...
    n_job_types = label_meta.get("n_job_types", 2)
...
def _save_checkpoint(model, norm_stats, label_meta, history, path=CHECKPOINT_PATH):
    serialisable_meta = {
