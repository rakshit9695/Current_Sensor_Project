"""
mv_conveyer/validation.py
Data-quality checks for the MV Conveyer current data.

Key differences vs wave_soldering:
- Phase imbalance tolerance is MUCH higher (1.5 CV vs 0.80):
  I2 legitimately drops to zero during no-load cycles on the conveyor.
  This is normal operating behaviour, not a sensor fault.
- min_current_floor is lower (0.05 A) — baseline conveyor draw is small
...
DEFAULT_THRESHOLDS = dict(
    ...
    min_current_floor     = 0.05,   # A  — conveyor baseline is low (~0.28 A avg)
    max_phase_imbalance   = 1.50,   # CV — I2 drops to 0 during no-load; expected
...
def validate_window(
    ...
    # 3. Phase consistency check (relaxed for conveyor)
    phase_rms_vals = [
