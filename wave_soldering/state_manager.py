...
class StateManager:
    ...
    def __init__(
        ...
        self._streak         = 0              # consecutive windows with _candidate
        self._history: list[StateEntry] = []  # confirmed transitions
    ...
    def update(
        ...
        Returns the *confirmed* current state (may differ from new_state
        while the streak counter is still building).
        """
        if new_state == self._candidate:
            self._streak += 1
        else:
            # New candidate – reset streak counter
    ...
    def run_batch(
        ...
        Useful for pre-computing the full timeline before playback.
        """
...
def build_state_timeline(
    ...
    window_centres,          # array-like of timestamps or floats
) -> list[dict]:
    """
    Build a compact run-length encoded timeline for the state bar chart.

    Returns a list of dicts with keys: start, end, state.
    Compatible with Plotly shapes / Gantt charts.
    """
