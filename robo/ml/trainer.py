"""
robo/ml/trainer.py
Training loop, multi-task loss, and checkpoint management for EPSON Robot.

Multi-task loss
---------------
  L = α·CE(state) + β·MSE(cycle_pos, masked) + γ·CE(job_type, masked)

Checkpoint saved to:  robo/ml/checkpoints/model.pt
"""
...
from .dataset import RoboDataset, NormStats
...
def multi_task_loss(
    ...
    cycle_pos_pred: torch.Tensor,
    job_logits:     torch.Tensor,
    state_true:     torch.Tensor,
    cycle_pos_true: torch.Tensor,
    job_true:       torch.Tensor,
    alpha: float = 1.0,
    beta:  float = 0.5,
    gamma: float = 0.5,
