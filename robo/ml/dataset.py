"""
robo/ml/dataset.py
PyTorch Dataset for the EPSON Robot iTransformer.

Single-phase adaptation:
  - Only 2 meaningful features: I1 (RMS current) and I_Avg (sensor average)
  - I2 and I3 are always 0 → excluded from feature set
  - Input tensor: (look_back, 2)  — I1, I_Avg (z-score normalised)

State label : int  0=STANDBY / 1=IDLE / 2=ACTIVE
...
class NormStats:
    ...
    mean: np.ndarray   # shape (2,)  – I1, I_Avg
...
class RoboDataset(Dataset):
    """
    Sliding look-back window dataset for EPSON Robot.
    ...
    def __getitem__(self, idx: int):
        ...
        x   = torch.from_numpy(self._x[idx:end])        # (look_back, 2)
        tgt = end - 1
...
def build_datasets(
    ...
    train_frac:  float = 0.70,
    val_frac:    float = 0.15,
) -> tuple[RoboDataset, RoboDataset, RoboDataset, NormStats]:
    train_df, val_df, test_df = temporal_split(df_labeled, train_frac, val_frac)
    norm_stats = NormStats.from_dataframe(train_df)
    train_ds = RoboDataset(train_df, look_back=look_back, norm_stats=norm_stats)
    val_ds   = RoboDataset(val_df,   look_back=look_back, norm_stats=norm_stats)
    test_ds  = RoboDataset(test_df,  look_back=look_back, norm_stats=norm_stats)
