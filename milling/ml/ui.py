...
milling/ml/ui.py
================
Streamlit ML page for the Milling Machine.
...
Single-file machine: labels, training, evaluation and inference all run on
the one milling dataset (no separate reference/operational split).
"""
...
from .predictor import MillingPredictor

_STATE_COLORS = {0: "#e74c3c", 1: "#f39c12", 2: "#27ae60"}
_STATE_NAMES  = {0: "Idle", 1: "No-Load", 2: "Cutting"}
_JOB_COLORS   = ["#4e9af1", "#f1914e", "#9b59b6", "#2ecc71", "#e74c3c"]
...
def _ss_set(key, val):
    st.session_state[f"mill_ml_{key}"] = val
...
def render_ml_page(df: pd.DataFrame):
    """
    Render the full ML diagnostics page for the Milling Machine.

    Parameters
    ----------
    df : cleaned milling DataFrame returned by data_loader.load_data
    """
    st.title("🤖 ML Diagnostics — Milling Machine")
    st.caption(
        "Auto-labels the idle → no-load → cutting cycle from raw current data, "
        "then trains an iTransformer to predict state, cycle position and "
...
def _tab_data_labels(df: pd.DataFrame):
    st.subheader("Label Generation Settings")

    with st.expander("⚙️ Threshold parameters", expanded=False):
        c1, c2, c3, c4 = st.columns(4)
        halt_max = c1.number_input("Idle max I_Avg (A)", min_value=0.1, max_value=3.0,
                                   value=DEFAULT_HALT_MAX, step=0.1, key="mill_halt_max")
        run_min  = c2.number_input("Cutting min I_Avg (A)", min_value=0.5, max_value=10.0,
                                   value=DEFAULT_RUN_MIN,  step=0.1, key="mill_run_min")
        min_run  = c3.number_input("Min run samples",    min_value=1,    max_value=60,
                                   value=DEFAULT_MIN_RUN,  step=1,    key="mill_min_run")
        n_jobs   = c4.number_input("Job clusters (K)",   min_value=2,    max_value=5,
                                   value=DEFAULT_N_JOBS,   step=1,    key="mill_n_jobs")

    regen = st.button("🔄 (Re)generate Labels", key="mill_regen_labels")
    ...
    c2.metric("Idle (RED)",      f"{n_halt:,}",  f"{100*n_halt/n_total:.1f}%")
    c3.metric("No-Load (AMBER)", f"{n_idle:,}",  f"{100*n_idle/n_total:.1f}%")
    c4.metric("Cutting (GREEN)", f"{n_run:,}",   f"{100*n_run/n_total:.1f}%")
    ...
    with col_hist:
        ...
        if durs:
            ...
            fig_hist.update_layout(
                title="Cutting Segment Durations (s)",
            ...
            st.info("No cutting cycles detected with current thresholds.")
    ...
    if cf_dict and "cycle_id" in cf_dict:
        ...
        if "job_type" in cf.columns and len(cf) > 0:
            st.subheader("Machining Operation Clusters (per cutting segment)")
            fig_sc = go.Figure()
            ...
            fig_sc.update_layout(
                title="Cluster: Duration vs Mean Current per Cutting Segment",
...
def _tab_train(df: pd.DataFrame):
    ...
    c1, c2, c3, c4 = st.columns(4)
    look_back  = c1.slider("Look-back window (s)", 10, 120, 30, 5,  key="mill_look_back")
    epochs     = c2.slider("Epochs",               5,  50,  20, 5,  key="mill_epochs")
    batch_size = c3.selectbox("Batch size", [32, 64, 128, 256], index=1, key="mill_batch")
    lr         = c4.select_slider("Learning rate", [1e-4, 3e-4, 1e-3, 3e-3], value=1e-3, key="mill_lr")

    c5, c6, c7 = st.columns(3)
    d_model    = c5.selectbox("d_model",  [32, 64, 128], index=1, key="mill_dmodel")
    n_layers   = c6.selectbox("n_layers", [2, 3, 4],     index=1, key="mill_nlayers")
    n_heads    = c7.selectbox("n_heads",  [2, 4, 8],     index=1, key="mill_nheads")

    c8, c9, c10 = st.columns(3)
    alpha = c8.number_input("α (state loss weight)",    min_value=0.1, max_value=5.0, value=1.0, step=0.1, key="mill_alpha")
    beta  = c9.number_input("β (cycle pos weight)",     min_value=0.0, max_value=2.0, value=0.5, step=0.1, key="mill_beta")
    gamma = c10.number_input("γ (job type weight)",     min_value=0.0, max_value=2.0, value=0.5, step=0.1, key="mill_gamma")
    ...
    if not st.button("🚀 Start Training", type="primary", key="mill_train_btn"):
        return

    # ── Build training dataset from the labelled milling data ─────────
    try:
        train_ds, val_ds, test_ds, norm_stats = build_datasets(df_lab, look_back=look_back)
...
def _tab_evaluate(df: pd.DataFrame):
    st.subheader("Model Evaluation on Milling Dataset")
    ...
    if not st.button("▶️ Run Evaluation", type="primary", key="mill_eval_btn"):
        return

    with st.spinner("Loading model and running inference…"):
        try:
            predictor = MillingPredictor.from_checkpoint(CHECKPOINT_PATH)
    ...
    rows = []
    for cls in ["Idle", "No-Load", "Cutting"]:
        r = rep.get(cls, {})
        rows.append({
            "Class":     cls,
    ...
    jcm = metrics["job_cm"]
    if jcm.sum() > 0:
        st.subheader("Operation-Type Confusion Matrix")
        ...
        fig_jcm.update_layout(
            title="Operation-Type Confusion Matrix",
            xaxis_title="Predicted", yaxis_title="True",
...
def _tab_inference(df: pd.DataFrame):
    ...
    start_s, end_s = st.slider(
        ...
        key="mill_infer_range",
    )

    if st.button("🔮 Run Inference on Selection", key="mill_infer_btn"):
    ...
    state_num = {"Idle": 0, "No-Load": 1, "Cutting": 2}
    ...
    fig.add_trace(go.Scatter(
        ...
        mode="markers", name="ML Operation",
        marker=dict(
    ...
    fig.update_yaxes(title_text="State (0-2)",  row=2, col=1,
                     tickvals=[0,1,2], ticktext=["Idle","No-Load","Cutting"])
