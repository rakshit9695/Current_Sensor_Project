...
Columns added to a copy of the input DataFrame:
  ml_state        : int   0/1/2   (Idle / No-Load / Cutting)
  ml_state_name   : str   "Idle" / "No-Load" / "Cutting"
  ml_state_conf   : float 0-1     (softmax probability of predicted class)
  ml_cycle_pos    : float 0-1     (predicted position in current cutting segment)
  ml_job_type     : int   0..K-1  (predicted machining operation cluster)
...
def _job_names(n: int) -> dict[int, str]:
    """
    Map cluster index → human-readable label.

    Clusters are ordered by mean log_duration (ascending), so cluster 0 =
    shorter/lower-load segments and cluster K-1 = longer/higher-load segments.
    For n=2: Light Load = short cyclic bursts; Heavy Load = sustained / high-current runs.
    """
    if n == 1:
        return {0: "Standard"}
    if n == 2:
...
class MillingPredictor:
    ...
    Usage
    -----
        predictor = MillingPredictor.from_checkpoint()
    ...
    @classmethod
    def from_checkpoint(cls, path: Path = CHECKPOINT_PATH) -> "MillingPredictor":
    ...
    def predict(
        ...
        First (look_back - 1) rows will have NaN / -1 predictions since no
        full window is available yet.
        """
    ...
    def evaluate_on_labeled(self, df_labeled: pd.DataFrame, batch_size: int = 256) -> dict:
        ...
        Requires 'state_label', 'cycle_pos', 'job_type' columns.
        """
        ...
        state_report = classification_report(
            ...
            labels=[0, 1, 2],
            target_names=["Idle", "No-Load", "Cutting"],
            output_dict=True,
            zero_division=0,
        )
