"""
windowing.py
Time-based sliding window generator for the current sensor data.

Since the data is sampled at 1 Hz (one RMS reading per second), a
...
def generate_windows(
    ...
    Notes
    -----
    * Works correctly even if the timestamp spacing is slightly irregular
      because we use time-based start/end boundaries, not fixed row counts.
    * Windows shorter than half the requested size (at the dataset boundary)
      are discarded.
    """
    if df.empty:
        return []
