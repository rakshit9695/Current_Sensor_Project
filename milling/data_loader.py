"""
milling/data_loader.py
Load and preprocess Milling Machine CSV data.

Key facts about the source data:
  - Three-phase spindle/feed drive: I1, I2, I3 all carry current
  - I_Avg column present in source; we recompute for consistency
  - Timestamps are ISO datetimes (no wrapping), nominally 1 Hz, with
    multi-hour idle gaps between machining sessions
  - The raw log contains a handful of corrupt rows (a concatenated
    timestamp glitch and a trailing blank line) whose currents are all
    zero — these are dropped on load
  - Current range: I_Avg ≈ 0.06 – 34 A
      Idle      : I_Avg < ~1 A   (machine off / control only)
      No-Load   : I_Avg ~1 – 3 A (spindle spinning, not cutting)
      Cutting   : I_Avg > ~3 A   (active material removal)
...
def _load_single(src) -> pd.DataFrame:
    ...
    # ── Parse ISO datetime timestamps (drop unparseable/corrupt rows) ──
    if "timestamp_str" in df.columns:
        df["timestamp"] = pd.to_datetime(
            df["timestamp_str"].str.strip(), format="ISO8601", errors="coerce"
        )
        n_bad = int(df["timestamp"].isna().sum())
        if n_bad:
            print(f"[milling.data_loader] Dropping {n_bad} corrupt/blank timestamp row(s).")
...
def load_data(source) -> pd.DataFrame:
    """
    Load Milling Machine data from one or more sources, return a clean DataFrame.
