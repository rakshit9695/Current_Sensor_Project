"""
milling/validation.py
Data-quality checks for the Milling Machine current data.

Key facts:
- Three-phase spindle/feed drive — all three phases carry current and stay
  reasonably balanced (CV typically ~0.13, p95 ~0.28), so the phase-imbalance
  tolerance is tight (0.80 CV), like the furnace.
- min_current_floor is low (0.05 A) — idle baseline draws very little.
...
DEFAULT_THRESHOLDS = dict(
    ...
    min_current_floor     = 0.05,   # A  — idle baseline is low
    max_phase_imbalance   = 0.80,   # CV — three balanced phases when running
